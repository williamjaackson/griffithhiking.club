...
server {
    ...
    # Set at server level because nginx's index handling internally redirects
    # / to /index.html, which leaves any `location = /` block behind. A
    # location declaring its own add_header does not inherit this one, which is
    # exactly how /_astro/ opts out below.
    ...
    # Follow the 'current' symlink rather than caching its old target.
